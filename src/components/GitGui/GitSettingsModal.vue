...
const loadKeyInfo = async () => {
  ...
  error.value = null;
  try {
    keyInfo.value = await invoke<SshKeyInfo>('get_ssh_key_info');
  } catch (err: any) {
...
const generateKey = async () => {
  ...
  error.value = null;
  try {
    keyInfo.value = await invoke<SshKeyInfo>('generate_ssh_key');
  } catch (err: any) {
    error.value = err.toString();
  } finally {
...
const clearAvatarCache = async () => {
  ...
  try {
    ...
    alert('Avatar cache cleared successfully.');
  } catch (err: any) {
    error.value = err.toString();
  } finally {
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content shadow-lg border-0">
        ...
        <div class="modal-body p-4">
          ...
          <div class="card shadow-sm mb-4 border-0 bg-light-subtle">
            <div class="card-header bg-transparent border-0 pt-3">
              <h6 class="mb-0 fw-bold">Appearance & Cache</h6>
            </div>
            <div class="card-body">
              <div class="d-flex align-items-center justify-content-between">
                <div>
                  <h6 class="mb-1 small fw-bold">Avatar Cache</h6>
                  <p class="text-muted smaller mb-0">Clear locally cached author images to refresh them from remotes.</p>
                </div>
                <button 
